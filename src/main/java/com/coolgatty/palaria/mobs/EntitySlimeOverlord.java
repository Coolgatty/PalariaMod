...
public class EntitySlimeOverlord extends EntityMob
{
    ...
    public EntitySlimeOverlord(World worldIn)
    {
        super(worldIn);
        this.moveSpeed = 0.9D;
    ...
    {        
        ...
        if (this.worldObj.isRemote)
        {
            for (int i = 0; i < 2; ++i)
            {
                this.worldObj.spawnParticle(EnumParticleTypes.WATER_SPLASH, this.posX + (this.rand.nextDouble() - 0.5D) * (double)this.width, this.posY + this.rand.nextDouble() * (double)this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * (double)this.width, 0.0D, 0.0D, 0.0D, new int[0]);
            }
        }
